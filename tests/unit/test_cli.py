...
class TestCLIArgumentParsing(unittest.TestCase):
    ...
    def test_cli_accepts_http_url_input(self):
        ...
        # Should not exit with usage error (exit code 2)
        self.assertNotEqual(result.exit_code, 2, 
                            "CLI should accept a valid HTTP(S) URL as input")
    ...
    def test_cli_validates_input_is_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotEqual(result.exit_code, 0,
                                "CLI should error if input is a directory")
            # Check for directory-related error message
            self.assertTrue(
                any(phrase in result.output.lower() for phrase in 
                    ['not a file', 'is a directory', 'invalid']),
